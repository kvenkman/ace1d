PRO ace_1d_chapman, model_sun, zcol, pconst, zmaj, model, mass, count
    ...
    ; Calculate vertical & slant column densities for a given zenith angle
    ; These are subsequently used for optical depth calculations in 
    ; ace_1d_solarphotoproc
    ...
    ; Thickness of each vertical bin
    ; (kT/mg)*dz (dz is the specified vertical model resolution)
    zcol.w = model.dz*(pconst.gask*zmaj.tn)/(pconst.grav*zmaj.barm)
    w = zcol.w

    ; Vertical column densities are calculated by integrating from top down
    o=zmaj.oden
    o2=zmaj.o2den
    n2=zmaj.n2den
